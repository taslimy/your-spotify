...
export const getTopArtTrack = () =>
  axios.get("https://api.spotify.com/v1/me/top/artists?time_range=long_term", {
    headers
  });

export const getUserInfo = () => {
  return axios.all([getUser(), getRecentlyPlayed(), getTopArtTrack()]).then(
    axios.spread((user, recentPlayed, getTopArtTrack) => {
      console.log("getTopArtTrack", getTopArtTrack.data.items);
      return {
        user: user.data,
        recentPlayed: recentPlayed.data.items,
        getTopArtTrack: getTopArtTrack.data.items
      };
    })
  );
};
